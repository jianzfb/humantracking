...
from .lite_resnet import *
from .ODFA import *


@CLASSIFIERS.register_module()
class ReidLiteClassifier(BaseClassifier):
    def __init__(self,
                 train_cfg=None,
                 init_cfg=None, **kwargs):
        super(BaseClassifier, self).__init__(init_cfg)
        class_num = train_cfg.get('class_num')
        self.model = LiteResnet(num_classes=class_num, in_channels=3)
        ...
        self.circle = circle
        classifier = ClassBlock(384, class_num, droprate, linear=linear_num, return_f = True)
        ...
        if train_cfg.get('sphere', False):
            self.criterion_sphere = losses.SphereFaceLoss(num_classes=class_num, embedding_size=512, margin=4)

        # 对抗间隔
        self.adv_iter = train_cfg.get('adv_iter', 10)
        # 对抗权重
        self.adv_weight = train_cfg.get('adv_weight', 0.0)
    ...
    def forward_train(self, image, label=None, **kwargs):
        ...
        logits, feature = self.model.classifier(x)

        if self.adv_weight > 0 and kwargs['iter'] % self.adv_iter==0: 
            inputs_adv = ODFA(self.model, image)
            outputs_adv = self.model(inputs_adv)
            outputs_adv = outputs_adv.view(outputs_adv.size(0), outputs_adv.size(1))
            logits_adv, _ = self.model.classifier(outputs_adv)
        ...
        if not return_feature:
            ...
            if self.adv_weight > 0 and kwargs['iter'] % self.adv_iter==0:
                adv_loss = self.adv_weight * self.criterion(logits_adv, label)
                losses.update({
                    'adv_loss', adv_loss
                })
