...
def debug_show(frame_index, resized_image, heatmap, offset, seg):
    image_h, image_w = resized_image.shape[:2]
    seg_map = cv2.resize(seg[0,1], (image_w, image_h))
...
def debug_showv2(frame_index, resized_image, pose,score):
    print(pose.shape)

    for (point_x,point_y), point_score in zip(pose, score):
        if point_score >= 0.03:
            resized_image = cv2.circle(resized_image, (int(point_x), int(point_y)), 1, (0,0,255),1)

    for s,e in skeleton:
        start_x,start_y = pose[s]
        start_score = score[s]
        end_x, end_y = pose[e]
        end_score = score[e]

        if start_x < 0 or end_x < 0 or start_score < 0.03 or end_score < 0.03:
            continue

        cv2.line(resized_image, (int(start_x), int(start_y)), (int(end_x), int(end_y)), (255,0,0), 1)

    if not os.path.exists('./temp'):
        os.makedirs('./temp')
    frame_index = 0
    cv2.imwrite(f'./temp/canvas_{frame_index}.png', resized_image)
    print('./')


decoder_simcc = SimCCLabel(
            input_size=(192,256),
            sigma=(4.9, 5.66),
...
def decode_rtmp(resized_image_for_pose, pred_x, pred_y):
    global decoder_simcc
    pose = decoder_simcc.decode(pred_x, pred_y)
...
video_dc['image', 'frame_index']('/workspace/project/sports/video/control/223_1699260485.mp4'). \
    ...
    inference_onnx_op['resized_image_for_pose', ('pred_x', 'pred_y')](
    ...
    runas_op[('resized_image_for_pose', 'pred_x', 'pred_y'), ('pose', 'score')](func=decode_rtmp). \
    runas_op[('frame_index','resized_image_for_pose', 'pose', 'score'), 'out'](func=debug_showv2).run()