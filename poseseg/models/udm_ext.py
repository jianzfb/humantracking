...
class UDM(nn.Module):
    ...
    def forward(self, x):
        ...
        x4 = self.backbone.layer1(x)            # 1/4

        out_layer_1 = self.layer4_1(x4)
        x8 = self.backbone.layer2(x4)           # 1/8
        ...
        x16 = self.backbone.layer3[0](x8)
        x16 = F.relu(x16 + self.down4_to_16(out_layer_2))
        x16 = self.backbone.layer3[1:](x16)
        ...
        x32 = self.backbone.layer4[0](x16)
        x32 = F.relu(x32 + self.down4_to_32(out_layer_3))
        x32 = self.backbone.layer4[1:](x32)
