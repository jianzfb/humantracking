...
from antgo.framework.helper.cnn import ConvModule, Scale

from models.utils.dist_utils import reduce_mean
from antgo.framework.helper.runner import BaseModule
from antgo.framework.helper.cnn.utils import bias_init_with_prob
from antgo.framework.helper.structures import InstanceData

from torch import Tensor

from models.utils.nms import nms_torch
from models.utils.misc import filter_scores_and_topk
from models.utils.transformer import (GAUEncoder, SinePositionalEncoding)
from antgo.framework.helper.models.builder import MODELS
from models.structures.bbox.transforms import bbox_xyxy2cs
from models.utils.typing import Features, OptSampleList, Predictions, ConfigType
...
class DCC(BaseModule):
    ...
    def _convert_forward_test(self):
        ...
        x_bins_.numpy().tofile('./pose_x_b.bin')
        y_bins_.numpy().tofile('./pose_y_b.bin')
        dim_t.numpy().tofile('./pose_dim.bin')
        ...
        def _forward_test(self, pose_feats, bbox_cs, grids):
            ...
            kpt_feats = self.pose_to_kpts(pose_feats)
            self.pose_to_kpts.weight.detach().cpu().numpy().tofile('./pose_to_kpts_weight.bin')
            self.pose_to_kpts.bias.detach().cpu().numpy().tofile('./pose_to_kpts_bias.bin')
            ...
            bbox_cs = torch.from_numpy(np.array([[276.81104, 117.69583, 38.44101, 135.95404]], dtype=np.float32))
            center, scale = bbox_cs.split(2, dim=-1)
            grids = torch.from_numpy(np.array([[279.5, 87.5]], dtype=np.float32))
            center = center - grids
            ...
            self.x_fc.weight.detach().cpu().numpy().tofile('./pose_x_fc_weight.bin')
            self.x_fc.bias.detach().cpu().numpy().tofile('./pose_x_fc_bias.bin')
            self.y_fc.weight.detach().cpu().numpy().tofile('./pose_y_fc_weight.bin')
            self.y_fc.bias.detach().cpu().numpy().tofile('./pose_y_fc_bias.bin')
...
class RTMOHead(YOLOXPoseHead):
    ...
    def __init__(
        ...
        if keypoint_weights is not None:
            ...
            if len(keypoint_weights.shape) == 1:
                keypoint_weights = keypoint_weights[None, :]
            self.keypoint_weights = torch.from_numpy(keypoint_weights)
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            losses['loss_vis'] = (self.loss_vis(kpt_vis_preds, vis_targets, vis_weights).sum(-1)/(vis_weights.sum(-1) + 1e-6)).mean()
            ...
            if hasattr(self, 'loss_mle') and self.loss_mle.loss_weight > 0:
                ...
                # 存在关键点没有进行标注，需要使用vis_weights过滤，有标注的点
                losses['loss_mle'] = ((self.loss_mle(pred_hms, target_hms, vis_targets)*vis_weights).sum(-1)/(vis_weights.sum(-1) + 1e-6)).mean()
            ...
            if self.proxy_target_cc:
                ...
                with torch.no_grad():
                    ...
                    kpt_weights_reg = vis_targets * mask * vis_weights
                ...
                if self.keypoint_weights.device != kpt_targets.device:
                    self.keypoint_weights = self.keypoint_weights.to(kpt_targets.device)
                losses['loss_oks'] = self.loss_oks(kpt_reg_preds, 
                                                   kpt_targets,
                                                   vis_weights * self.keypoint_weights, pos_areas)
    ...
    def predict(self,
        ...
        # TODO, 没有使用 DCC
        cls_scores, bbox_preds, kpt_offset, kpt_vis, _ = self.forward(feats)
        ...
        flatten_kpt_offset = self._flatten_predictions(kpt_offset)
        # flatten_pose_vecs = self._flatten_predictions(pose_vecs)
        ...
        flatten_kpt_decoded = \
            self.decode_kpt_reg(flatten_kpt_offset,flatten_priors, flatten_stride)

        results_list = []
        for (bboxes, scores, kpt_vis, kpt,
             img_meta) in zip(flatten_bbox_preds, flatten_cls_scores,
                              flatten_kpt_vis, flatten_kpt_decoded,
                              batch_img_metas):

            score_thr = cfg.get('score_thr', 0.3)
            ...
            kpt = kpt[keep_idxs_score]

            if bboxes.numel() > 0:
                nms_thr = cfg.get('nms_thr', 0.3)
    ...
    def switch_to_deploy(self, test_cfg: Optional[Dict]):
        ...
        # grid generator
        input_size = test_cfg.get('input_size', (384, 384))
        featmaps = []
