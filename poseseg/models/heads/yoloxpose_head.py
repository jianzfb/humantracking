...
from antgo.framework.helper.models.builder import MODELS, LOSSES
from antgo.framework.helper.models import *
from antgo.framework.helper.models.detectors.core import PRIOR_GENERATORS,BBOX_ASSIGNERS

from models.structures.pose_data_sample import PoseDataSample
from models.utils.dist_utils import reduce_mean
from models.utils.typing import (ConfigType, Features, OptSampleList,
...
class YOLOXPoseHead(BaseModule):
    ...
    def __init__(
        ...
        self.prior_generator = PRIOR_GENERATORS.build(prior_generator)
        ...
        if loss_obj is not None:
            self.loss_obj = LOSSES.build(loss_obj)
        self.loss_bbox = LOSSES.build(loss_bbox)
        self.loss_oks = LOSSES.build(loss_oks)
        self.loss_vis = LOSSES.build(loss_vis)
        if loss_bbox_aux is not None:
            self.loss_bbox_aux = LOSSES.build(loss_bbox_aux)
        if loss_kpt_aux is not None:
            self.loss_kpt_aux = LOSSES.build(loss_kpt_aux)
