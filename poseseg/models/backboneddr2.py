...
class BackboneDDR2(nn.Module):
    def __init__(self, architecture='resnet34', in_channels=1, with_aspp=True):
        super(BackboneDDR2, self).__init__()
        ...
        self.block.exp = 6
        self.layer7 = self.make_layer(self.block, 128, self.layers[6], stride=1, no_final_relu=True)
        self.layer8 = self.make_layer(self.block, 160, self.layers[7], stride=2, no_final_relu=True)

        self.aspp = None
        self.aspp_fuse = None
        if with_aspp:
            self.aspp = ASPPModule([1, 2, 4, 6], 160, 40, conv_cfg=None, norm_cfg=dict(type='BN'), act_cfg=dict(type='ReLU'))
            self.aspp_fuse = ConvModule(160,160,3,padding=1,conv_cfg=None,norm_cfg=dict(type='BN'),act_cfg=dict(type='ReLU'))
    ...
    def forward(self, x):
        ...
        out_layer_2 = \
            self.layer4_2(out_layer_1) + F.interpolate(
                                    ...
                                    scale_factor=2,
                                    mode='bilinear')
        out_layer_2 = F.relu(out_layer_2)
        ...
        # aspp
        if self.aspp is not None and self.aspp_fuse is not None:
            x4 = self.aspp(x4)
            x4 = self.aspp_fuse(torch.cat(x4, dim=1))
