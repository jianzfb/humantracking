...
class PoseSegDDRNetV2(BaseModule):
    def __init__(self, backbone, train_cfg=None, test_cfg=None, init_cfg=None):
        ...
        self.final_offset_layer_2 = nn.Sequential(
            nn.Conv2d(decoder_channels+2, decoder_channels, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(decoder_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(decoder_channels, decoder_channels, kernel_size=3, stride=1, padding=1),
    ...
    def forward_train(self, image, heatmap, offset_x, offset_y, heatmap_weight, joints_vis, segments, **kwargs):
        ...
        layout_2_uv_heatmap = self.final_heatmap_layer_2(output_layout_2)
        layout_2_uv_off = self.final_offset_layer_2(output_layout_2)

        layout_2_uv_heatmap = layout_2_uv_heatmap[kwargs['has_joints']]
        layout_2_uv_off = layout_2_uv_off[kwargs['has_joints']]

        layout_2_loss_output = \
            self._compute_loss_with_heatmap(
                layout_2_uv_heatmap, layout_2_uv_off, heatmap, heatmap_weight, offset_x, offset_y, joints_vis)
        ...
        loss_output = {
            'layout_1_loss_uv_hm': layout_1_loss_output['loss_uv_hm'],
            'layout_1_loss_xy_offset': layout_1_loss_output['loss_xy_offset'],
            'layout_1_loss_seg': layout_1_seg_loss_value,
            'layout_2_loss_uv_hm': layout_2_loss_output['loss_uv_hm'],
