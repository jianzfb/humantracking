...
class PoseSegDDRNetV5(BaseModule):
    ...
    def forward_train(self, image, heatmap, offset_x, offset_y, heatmap_weight, joints_vis, segments, **kwargs):
        ...
        pose_att_w = self.pose_att(pose_feature_map)
        pose_feature_map_c_att = pose_feature_map * pose_att_w
        coarse_pose_uv_heatmap = self.pose_heatmap_coarse_layer(pose_feature_map_c_att)
        coarse_pose_uv_heatmap = coarse_pose_uv_heatmap[kwargs['has_joints']]
        ...
        seg_att_w = self.seg_att(seg_feature_map)
        seg_feature_map_c_att = seg_feature_map * seg_att_w
        seg_logits_coarse = self.seg_coarse_layer(seg_feature_map_c_att)
        coarse_seg_logits = seg_logits_coarse[kwargs['has_segments']]
        ...
        with torch.no_grad():
            no_grad_cross_pose_att_w = self.seg_att(pose_feature_map)
        pose_feature_map_cross_c_att = pose_feature_map * no_grad_cross_pose_att_w
        pose_feature_map = torch.concatenate([pose_feature_map_c_att, pose_feature_map_cross_c_att], 1)
        ...
        #--------------------refine seg loss-----------------------------#
        with torch.no_grad():
            no_grad_cross_seg_att_w = self.pose_att(seg_feature_map)
        seg_feature_map_cross_c_att = seg_feature_map * no_grad_cross_seg_att_w
        seg_feature_map = torch.concatenate([seg_feature_map_c_att, seg_feature_map_cross_c_att], 1)
    ...
    def forward_test(self, image, **kwargs):
        ...
        seg_feature_map_cross_c_att = seg_feature_map * no_grad_cross_seg_att_w
        seg_feature_map = torch.concatenate([seg_feature_map_c_att, seg_feature_map_cross_c_att], 1)
        refine_seg_logits = self.seg_coarse_layer(seg_feature_map)
