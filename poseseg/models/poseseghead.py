...
class PoseHS2D(nn.Module):
    def __init__(self, num_deconv_filters, num_joints):
        super(PoseHS2D, self).__init__()
        ...
        self.fuse_up16 = nn.Sequential(
            nn.Conv2d(self.deconv_dim[1], self.deconv_dim[1], kernel_size=3, stride=1, padding=1),
        ...
        #     nn.Conv2d(self.deconv_dim[2], self.deconv_dim[2], kernel_size=3, stride=1, padding=1),
        ...
        self.final_layer = nn.Conv2d(
            self.deconv_dim[2], num_joints, kernel_size=3, stride=1, padding=1
        )  # up32 = torch.cat[up32, x32] x32's channel is 32
        self.final_offset_layer = nn.Conv2d(
            self.deconv_dim[2], num_joints * 2, kernel_size=3, stride=1, padding=1
        )  # up32 = torch.cat[up32, x32]  x32's channel is 32
        self.final_seg_layer = nn.Conv2d(
            self.deconv_dim[2], 2, kernel_size=3, stride=1, padding=1
        )
    ...
    def _make_deconv_layer(self):
        ...
        # deconv2 = nn.Upsample(scale_factor=2, mode='nearest')
        conv3_1x1 = nn.Conv2d(self.deconv_dim[1], self.deconv_dim[2], kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, *args):
        ...
        up32 = self.deconv_layer2(up16)

        uv_heatmap = self.final_layer(up32)
        uv_off = self.final_offset_layer(up32)
        seg = self.final_seg_layer(up32)
        return [uv_heatmap, uv_off, seg]
