...
class PyMAF(BaseModule):
    ...
    def __init__(self, train_cfg=None, **kwargs):
        ...
        self.global_mode = not self.train_cfg.MAF_ON
        self.feature_extractor = get_resnet_encoder(self.train_cfg.POSE_RES_MODEL, global_mode=self.global_mode)

        # deconv layers
        self.inplanes = self.feature_extractor.inplanes
        self.deconv_with_bias = self.train_cfg.RES_MODEL.DECONV_WITH_BIAS
        self.deconv_layers = self._make_deconv_layer(
            self.train_cfg.RES_MODEL.NUM_DECONV_LAYERS,
            self.train_cfg.RES_MODEL.NUM_DECONV_FILTERS,
            self.train_cfg.RES_MODEL.NUM_DECONV_KERNELS,
        )

        self.maf_extractor = nn.ModuleList()
        for _ in range(self.train_cfg.N_ITER):
            self.maf_extractor.append(MAF_Extractor(self.train_cfg.MLP_DIM))
        ma_feat_len = self.maf_extractor[-1].Dmap.shape[0] * self.train_cfg.MLP_DIM[-1]
        ...
        grid_feat_len = grid_size * grid_size * self.train_cfg.MLP_DIM[-1]

        self.regressor = nn.ModuleList()
        for i in range(self.train_cfg.N_ITER):
            if i == 0:
                ref_infeat_dim = grid_feat_len
            else:
                ref_infeat_dim = ma_feat_len
            self.regressor.append(Regressor(batch_size=self.train_cfg.batch_size, feat_dim=ref_infeat_dim, smpl_mean_params=SMPL_MEAN_PARAMS))

        dp_feat_dim = 256
        self.with_uv = self.train_cfg.LOSS.POINT_REGRESSION_WEIGHTS > 0
        if self.train_cfg.AUX_SUPV_ON:
            self.dp_head = nn.ModuleList()
            self.dp_head.append(IUV_predict_layer(feat_dim=dp_feat_dim))

        # calculate loss paramters
        self.criterion_shape = nn.L1Loss()
        self.criterion_keypoints = nn.MSELoss(reduction='none')
        self.criterion_regr = nn.MSELoss()
        self.img_res = self.train_cfg.IMG_RES
        self.focal_length = self.train_cfg.FOCAL_LENGTH
    ...
    def loss(self, image, preds_dict, gt_dict):
        ...
        gt_cam_t = estimate_translation(gt_model_joints, gt_keypoints_2d_orig, focal_length=self.focal_length, img_size=self.img_res)
        ...
        if self.train_cfg.AUX_SUPV_ON:
            ...
            gt_dict['iuv_image_gt'] = iuv_image_gt
    ...
    def forward_train(self, image, J_regressor=None, **kwargs):
        batch_size = image.shape[0]
        ...
        # dense supervised
        if self.train_cfg.AUX_SUPV_ON:
            iuv_out_dict = self.dp_head[0](s_feat)
