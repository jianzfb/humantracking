...
def cross_attention(query, key):
    ...
    # 对注意力分数进行缩放
    scaling_factor = 1.0 / (key.size(-1) ** 0.5)
    ...
    # 使用注意力权重对value进行加权求和
    value = key  # 假设value和key相同
    output = torch.matmul(attention_weights, value)
    return output

if __name__ == '__main__':
    query = torch.randn(1, 197, 768)
    key = torch.randn(1, 49, 768)

    net = Transformer_Block(768)
    query_c = net(query)

    # 使用交叉注意力函数计算注意力输出
    output = cross_attention(query, key)

    print("Cross Attention Output Tensor:")
    print(output.shape)