...
class CrossM(nn.Module):
    ...
    def forward(self, high_x1, low_x2):
        high_att = torch.sigmoid(low_x2)
        high_x1 = high_x1 * high_att
        high_x1 = self.cross_l(high_x1)
...
class DeltaM(nn.Module):
    ...
    def forward(self, x1, x2):
        x1 = F.interpolate(
                x1, scale_factor=2, mode='bilinear'
            )
...
class UDMDirAttLite(nn.Module):
    def __init__(self, in_channels=1, mid_channels=64, out_channels=64, delta_mode=False, pan_mode=True, deepen_factor=1.0, widen_factor=1.0, out_strides=[16,32]):
        super(UDMDirAttLite, self).__init__()
    ...
    def forward(self, x):
        ...
        x8 = x8 * self.down4_to_16(out_layer_2)
        ...
        x4 = x4 * self.down4_to_32(out_layer_3)
        ...
        if self.pan_mode:
            # 1/8
            downsample_feat = self.downsample_convs[0](out_layer_4)
        ...
        if self.delta_mode:
            feature_1_8, feature_1_16, feature_1_32 = out_list
            feature_1_16 = self.delta_conv_1(feature_1_32, feature_1_16)
            feature_1_8 = self.delta_conv_2(feature_1_16, feature_1_8)
            out_list = [feature_1_8, feature_1_16, feature_1_32]
