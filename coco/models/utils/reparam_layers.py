...
from antgo.framework.helper.cnn import ConvModule, SELayer, build_activation_layer, build_norm_layer
...
class RepVGGBlock(BaseModule):
    ...
    def __init__(self,
        ...
        if use_se:
            self.se = SELayer(out_channels, squeeze_channels=out_channels // 16)
        else:
            self.se = nn.Identity()
    ...
    def switch_to_deploy(self, test_cfg: Optional[Dict] = None):
        ...
        def _forward(self, x):
            return self.act(self.se(self.conv_reparam(x)))
