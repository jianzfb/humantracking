...
from models.boxes import bboxes_iou, cxcywh2xyxy
from models.compat import meshgrid
from models.demo_utils import visualize_assign
from antgo.framework.helper.cnn.bricks import ConvModule
from antgo.framework.helper.models.detectors.losses import *
...
class YOLOXHead(nn.Module):
        ...
        for i in range(len(in_channels)):
            self.stems.append(
                ConvModule(
                    ...
                    out_channels=int(self.out_channels * width),    # 256 * width
                    kernel_size=1,
                    stride=1,
            ...
            self.cls_convs.append(
                nn.Sequential(
                    *[
                        ConvModule(
                            in_channels=int(self.out_channels * width),     # 256 * width
                            out_channels=int(self.out_channels * width),    # 256 * width
                            kernel_size=3,
                            stride=1,
                            act_cfg=dict(type='ReLU'),
            ...
            self.cls_preds.append(
                nn.Conv2d(
                    in_channels=int(self.out_channels * width),
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            num_gt = labels[batch_idx].shape[0]
            num_gts += num_gt
            if num_gt == 0:
                ...
                is_assignment_ok = False
                try:
                    result  = self.get_assignments(  # noqa
                        batch_idx,
                        num_gt,
                        gt_bboxes_per_image,
                        gt_classes,
                    ...
                    if result is not None:
                        is_assignment_ok = True
                        gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg_img = result
                    else:
                        cls_target = outputs.new_zeros((0, self.num_classes))
                        reg_target = outputs.new_zeros((0, 4))
                        l1_target = outputs.new_zeros((0, 4))
    ...
    def visualize_assign_result(self, xin, labels=None, imgs=None, save_prefix="assign_vis_"):
        ...
        for batch_idx, (img, num_gt, label) in enumerate(zip(imgs, nlabel, labels)):
            ...
            logging.info(f"save img to {save_name}")