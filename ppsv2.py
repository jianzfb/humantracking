...
def debug_show(frame_index, resized_image, seg):
    image_h, image_w = resized_image.shape[:2]
    seg_map = cv2.resize(seg[0,0], (image_w, image_h))
    ...
    # resized_image = pose_draw(resized_image, xy)
...
video_dc['image', 'frame_index']('/workspace/project/sports/video/longjump/20240123175617.ts'). \
    ...
    inference_onnx_op['resized_image_for_person', 'peson_ball_output'](
        onnx_path='/workspace/humantracking/models/personball/coco-epoch_111-model.onnx', 
        mean=[128, 128, 128],
        std=[128, 128, 128],
        engine='rknn',
        engine_args={
    ...
    deploy.detpostprocess_func[('image', 'peson_ball_output'), ('person_bboxes', 'person_labels')](level_hw=np.array([32,48,16,24,8,12], dtype=np.int32), level_strides=np.array([8,16,32], dtype=np.int32)). \
    ...
    runas_op[('frame_index','resized_image_for_pose', 'seg'), 'out'](func=debug_show).run()